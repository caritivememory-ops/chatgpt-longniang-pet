Create one horizontal look-direction strip for Codex pet `chatgpt-longniang`, atlas row 9.
...
RIGHT-HALF LANDMARK LOCK: frames 2-8 (`022.5` through `157.5`) must never read screen-left. Progressively move the nose tip and face plane toward the SCREEN-RIGHT side of the head center; move both complete irises/pupils toward the screen-right sides of their original apertures; reveal the appropriate near cheek/ear/horn and progressively occlude the far side with the nose, bangs and skull. At `090`, use an unmistakable screen-right three-quarter/profile silhouette. The canonical screen-right flower-ornament side may become more visible as the head turns right, but the ornament must not swap sides. For `000`, put both irises/pupils high with visible eye-fill below, lift the lids/brows and chin, and make UP unmistakable rather than neutral.
